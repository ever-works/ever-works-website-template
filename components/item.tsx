...
import { FiStar, FiFolder, FiExternalLink } from "react-icons/fi";
...
export default function Item(props: ItemProps) {
  ...
  return (
    <Card
      className={cn(
        "group relative border transition-all duration-300 transform hover:-translate-y-1 backdrop-blur-sm overflow-hidden",
        // Light mode
    ...
    >
      ...
      <CardHeader className="relative z-10 flex gap-4 p-6 pb-4">
        ...
        <div className="flex-shrink-0">
          <div className="relative w-12 h-12 flex items-center justify-center rounded-xl transition-all duration-300 bg-red-100 border border-red-200 group-hover:bg-red-200 dark:bg-red-500/20 dark:border-red-500/30 dark:group-hover:bg-red-500/30">
            {props.icon_url ? (
              <Image
                src={props.icon_url}
                alt={`${props.name} icon`}
              ...
              <FiFolder className="w-6 h-6 text-red-600 dark:text-red-400" />
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col flex-grow gap-3 min-w-0">
          <div className="flex justify-between items-start gap-3">
            ...
            {props.featured && (
              <Badge
                color="primary"
                variant="flat"
                className="px-2 py-1 text-xs font-medium bg-blue-100 text-blue-700 border border-blue-200 dark:bg-blue-500/20 dark:text-blue-300 dark:border-blue-500/30"
              >
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className="px-3 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-700 border border-blue-200 dark:bg-blue-500/20 dark:text-blue-300 dark:border-blue-500/30">
              {getCategoriesName(props.category)}
            </span>
            {props.tags && Array.isArray(props.tags) && props.tags.slice(0, 2).map((tag, index) => {
              const tagName = getTagName(tag);
              if (!tagName) return null;
              return (
                <span key={index} className="px-3 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-700 border border-blue-200 dark:bg-blue-500/20 dark:text-blue-300 dark:border-blue-500/30">
                  {tagName}
                </span>
              );
            })}
