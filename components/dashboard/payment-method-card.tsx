"use client";

import { Edit, Trash2, Star, Calendar, Shield } from "lucide-react";
import { useTranslations } from "next-intl";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { PaymentMethod } from "./billing-section";

interface PaymentMethodCardProps {
  paymentMethod: PaymentMethod;
  onEdit: () => void;
  onDelete: () => void;
  onSetDefault: () => void;
}

const getCardIcon = (type: PaymentMethod["type"]) => {
  switch (type) {
    case "visa":
      return "💳";
    case "mastercard":
      return "💳";
    case "amex":
      return "💳";
    case "discover":
      return "💳";
    default:
      return "💳";
  }
};

const getCardColor = (type: PaymentMethod["type"]) => {
  switch (type) {
    case "visa":
      return "from-blue-500 to-blue-600";
    case "mastercard":
      return "from-red-500 to-red-600";
    case "amex":
      return "from-green-500 to-green-600";
    case "discover":
      return "from-orange-500 to-orange-600";
    default:
      return "from-gray-500 to-gray-600";
  }
};

const formatExpiryDate = (month: number, year: number) => {
  return `${month.toString().padStart(2, '0')}/${year.toString().slice(-2)}`;
};

const formatCreatedDate = (dateString: string) => {
  const date = new Date(dateString);
  return date.toLocaleDateString('fr-FR', {
    year: 'numeric',
    month: 'long',
    day: 'numeric'
  });
};

export function PaymentMethodCard({
  paymentMethod,
  onEdit,
  onDelete,
  onSetDefault
}: PaymentMethodCardProps) {
  const t = useTranslations("billing");
  const { type, last4, expiryMonth, expiryYear, holderName, isDefault, brand, createdAt } = paymentMethod;

  return (
    <div className="relative group">
      <div className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700 rounded-lg p-6 border border-gray-200 dark:border-gray-600 hover:shadow-md transition-all duration-200">
        <div className="flex items-start justify-between">
          <div className="flex items-start space-x-4 flex-1">
            <div className={`w-12 h-8 bg-gradient-to-r ${getCardColor(type)} rounded-md flex items-center justify-center text-white text-lg font-bold shadow-sm`}>
              {getCardIcon(type)}
            </div>

            <div className="flex-1">
              <div className="flex items-center space-x-2 mb-1">
                <h4 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                  {brand} •••• {last4}
                </h4>
                {isDefault && (
                  <Badge variant="secondary" className="bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200">
                    <Star className="h-3 w-3 mr-1" />
                    {t("DEFAULT")}
                  </Badge>
                )}
              </div>

              <div className="space-y-1">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  {holderName}
                </p>
                <div className="flex items-center space-x-4 text-sm text-gray-500 dark:text-gray-400">
                  <div className="flex items-center space-x-1">
                    <Calendar className="h-3 w-3" />
                    <span>{t("EXPIRES", { date: formatExpiryDate(expiryMonth, expiryYear) })}</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <Shield className="h-3 w-3" />
                    <span>{t("ADDED_ON", { date: formatCreatedDate(createdAt) })}</span>
                  </div>
                </div>
              </div>
            </div>
          </div>

          {/* Actions */}
          <div className="flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
            {!isDefault && (
              <Button
                variant="outline"
                size="sm"
                onClick={onSetDefault}
                className="text-xs"
              >
                {t("SET_AS_DEFAULT")}
              </Button>
            )}
            <Button
              variant="outline"
              size="sm"
              onClick={onEdit}
              className="p-2"
            >
              <Edit className="h-3 w-3" />
            </Button>
            <Button
              variant="outline"
              size="sm"
              onClick={onDelete}
              className="p-2 text-red-600 hover:text-red-700 hover:bg-red-50 dark:text-red-400 dark:hover:text-red-300 dark:hover:bg-red-900/20"
            >
              <Trash2 className="h-3 w-3" />
            </Button>
          </div>
        </div>

        {(() => {
          const currentDate = new Date();
          const expiryDate = new Date(expiryYear, expiryMonth - 1);
          const monthsUntilExpiry = (expiryDate.getFullYear() - currentDate.getFullYear()) * 12 + 
                                   (expiryDate.getMonth() - currentDate.getMonth());
          
          if (monthsUntilExpiry < 0) {
            return (
              <div className="mt-3 p-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md">
                <p className="text-xs text-red-800 dark:text-red-200 font-medium">
                  {t("CARD_EXPIRED")}
                </p>
              </div>
            );
          } else if (monthsUntilExpiry <= 2) {
            return (
              <div className="mt-3 p-2 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-md">
                <p className="text-xs text-yellow-800 dark:text-yellow-200 font-medium">
                  {t("CARD_EXPIRES_SOON", { months: monthsUntilExpiry })}
                </p>
              </div>
            );
          }
          return null;
        })()}
      </div>
    </div>
  );
}
