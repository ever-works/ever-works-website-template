...
const CategoryButton = memo(
	React.forwardRef<HTMLDivElement, CategoryButtonProps>(
		({ href, isActive, displayName, count, isTextTruncated, fullName, onClick }, ref) => {
		...
		<div className="space-y-3 sm:space-y-5">
			...
			<div className="hidden md:block">
				<div
					...
					className={
						showAllCategories ? 'relative pb-4 pr-8 transition-all duration-500' : SCROLL_CONTAINER_STYLES
					}
					role="region"
	...
	const renderCategory = (category: Category, index: number) => {
		...
		return (
			<Button
				...
				asChild
				className={cn(
					'px-1.5 rounded-full py-1 h-8 font-medium transition-all duration-200',
					isActive
		...
		<div
			className={cn(
				...
				props.enableSticky
					? cn(
							'sticky top-4 z-10',
							isSticky ? 'bg-white/95 dark:bg-gray-800/95 shadow-md backdrop-blur-sm' : 'bg-transparent'
						)
					: 'bg-inherit'
			)}
		>
				...
				<div className="flex items-center justify-between mb-3">
					<h3
						className={cn(
							...
							isSticky ? 'text-theme-primary dark:text-theme-primary' : 'text-gray-900 dark:text-white'
						)}
					>
						Categories
					</h3>
					{hasMoreTags && (
						<Button
							...
							size="sm"
							className={cn(
								'px-4 py-1 font-medium transition-all duration-300  text-theme-primary bg-theme-primary-10 hover:to-theme-primary rounded-full',
								isSticky && 'shadow-sm'
							)}
					...
					{!showAllCategories && (
						<div className="w-full flex flex-nowrap gap-2 overflow-x-auto pb-2 hide-scrollbar scrollbar-thin scrollbar-thumb-blue-500/30 dark:scrollbar-thumb-blue-700/30 scrollbar-track-transparent">
							<Button
								...
								asChild
								className={cn(
									'px-3 py-1 h-8 font-medium transition-all duration-300 shrink-0 group rounded-full',
									!isAnyTagActive
							...
							>
								<Link href={props.resetPath || props.basePath || '/'}>
									{!isAnyTagActive && (
										<svg
											className="w-3 h-3 mr-1.5 text-white"
											fill="none"
