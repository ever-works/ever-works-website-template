					...
					<div className="p-6 space-y-4">
						{items.map((item) => {
							...
							const isProcessingThisItem =
								pendingItemId === item.id && (isApproving || isRejecting || isDeleting);
												...
												{item.status === 'pending' &&
													(() => {
														const isApprovingThis =
															isApproving && pendingItemId === item.id;
														const isRejectingThis =
															isRejecting && pendingItemId === item.id;
														const isDeletingThis = isDeleting && pendingItemId === item.id;
														const isProcessingThis =
															isApprovingThis || isRejectingThis || isDeletingThis;
														...
														return (
															<>
																...
																<Button
																	...
																	title={
																		isRejectingThis ? t('REJECTING') : t('REJECT')
																	}
																>
																	{isRejectingThis ? (
																		<Loader2 size={14} className="animate-spin" />
