 ...
 * @param type - Filter by activity type ('all', 'comment', 'vote')
 * @returns Paginated activity list and total count
 */
export async function getUserActivityPaginated(
    clientProfileId: string,
    page: number = 1,
    limit: number = 10,
    type: 'all' | 'comment' | 'vote' = 'all'
): Promise<{ activities: UserActivityItem[]; total: number }> {
    ...
    const voteActivities: UserActivityItem[] = userVotes.map(vote => ({
        id: vote.id,
        itemId: vote.itemId,
    ...
    const commentActivities: UserActivityItem[] = userComments.map(comment => ({
        id: comment.id,
